...
#         print('image',image.size())
        ...
        _, _, out, _ = self.hidden_conv({0: v,
                                         ...
                                         3: label}).values()
#         print('out',out.size())
        latent_embedded = self.latent_mapping(label).unsqueeze(2).unsqueeze(3) ##128x30x1x1
#         print(latent_embedded.size())
        # add label bias
        out += latent_embedded  
        out = F.relu(out)
...
#                         samples[:, c, i, j] = unnormalized_probs[:, c, i, j]
#                         print(samples[:, c, i, j].size())
